...
canny::canny(string filename) {
    ...
    if (!img.data()) // Check for invalid input
    {
        ...
        img.save(FOLDER"result/ori.bmp");

        grayscaled = toGrayScale(); //Grayscale the image
        grayscaled.save(FOLDER"result/grayScaled.bmp");

        gFiltered = useFilter(grayscaled, filter); //Gaussian Filter
        gFiltered.save(FOLDER"result/gaussianFilter.bmp");

        sFiltered = sobel(); //Sobel Filter
        sFiltered.save(FOLDER"result/sobelFilter.bmp");

        non = nonMaxSupp(); //Non-Maxima Suppression
        non.save(FOLDER"result/nonMaximaSuppression.bmp");

        thres = threshold(non, 20, 40); //Double Threshold and Finalize
        thres.save(FOLDER"result/doubleThresholdAndFinalize.bmp");
    }
}

CImg<unsigned char> canny::toGrayScale() {
    grayscaled = CImg<unsigned char>(img.width(), img.height(), 1, 1); //To one channel
...
CImg<unsigned char> canny::sobel() {
    ...
    CImg<unsigned char> filteredImg(gFiltered.width() - 2 * size, gFiltered.height() - 2 * size, 1, 1);
        ...
        for (int j = size; j < gFiltered.height() - size; j++) {
            ...
            for (int x = 0; x < xFilter.size(); x++)
                for (int y = 0; y < xFilter.size(); y++) {
                    sumx += xFilter[x][y] *
                            (double) (gFiltered(i + x - size, j + y - size)); //Sobel_X Filter Value
                    sumy += yFilter[x][y] *
                            (double) (gFiltered(i + x - size, j + y - size)); //Sobel_Y Filter Value
                }
    ...
    for (int i = 1; i < sFiltered.width() - 1; i++) {
        for (int j = 1; j < sFiltered.height() - 1; j++) {
            ...
            if (((-22.5 < Tangent) && (Tangent <= 22.5)) || ((157.5 < Tangent) && (Tangent <= -157.5))) {
                if ((sFiltered(i, j) < sFiltered(i, j + 1)) ||
                    (sFiltered(i, j) < sFiltered(i, j - 1)))
                    nonMaxSupped(i - 1, j - 1) = 0;
            }
            //Vertical Edge
            if (((-112.5 < Tangent) && (Tangent <= -67.5)) || ((67.5 < Tangent) && (Tangent <= 112.5))) {
                if ((sFiltered(i, j) < sFiltered(i + 1, j)) ||
            ...
            if (((-67.5 < Tangent) && (Tangent <= -22.5)) || ((112.5 < Tangent) && (Tangent <= 157.5))) {
                if ((sFiltered(i, j) < sFiltered(i - 1, j + 1)) ||
